 ...
 */
public class SocialSentPostBySocialNet extends GenericResource{
    /**
     * The log.
     */
    private static Logger log = SWBUtils.getLogger(SocialSentPostBySocialNet.class);
    /**
     * The webpath.
     */
    ...
    public void doEdit(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        SocialNetwork socialNetwork = (SocialNetwork) SemanticObject.createSemanticObject(id).getGenericInstance();      
        ...
        HashMap hmapResult = filtros(swbSocialUser, wsite, searchWord, request, socialNetwork, nPage);
        long numSocialTopicPOComments=0L;
        String snumSocialTopicPOComments=getSumTotPostOutComments(socialNetwork);
        ...
        out.println("<a href=\"#\" class=\"countersBar\" title=\"Refrescar Tab\" onclick=\"submitUrl('" + urlRefresh.setMode(SWBResourceURL.Action_EDIT) + "',this); return false;\">"+nf2.format(numSocialTopicPOComments)+" mensajes enviados</a>");
        ...
        out.println("<th class=\"mensaje\">");
        out.println(paramRequest.getLocaleString("postTopic"));
        out.println("</th>");
        ...
        Iterator<PostOut> itposts = socialNetwork.listSocialNetworkPostOutInvs();//(Iterator)hmapResult.get("itResult"); 
        ...
        while (itposts!=null && itposts.hasNext()) {
            ...
            //Show Topic
            out.println("<td>");
            out.println(postOut.getSocialTopic().getDisplayTitle(paramRequest.getUser().getLanguage()));
            out.println("</td>");
            ...
            if (!postOut.isPublished()) {
                ...
                if (!isInFlow && postOutwithPostOutNets && !someOneIsNotPublished) //Se supone que por lo menos, hay publicado un PostOutNet del Post                         
                {
                    ...
                    postOut.setPublished(true);
                    out.println("<a class=\"status3\" href=\"#\" title=\"" + paramRequest.getLocaleString("postOutLog") + "\"><strong>Publicado</strong></a>"); //Publicado
                } else {
                    ...
                    if (!needAuthorization || postOut.getPflowInstance().getStatus() == 3) {
                        ...
                        if (someOneIsNotPublished) {
                            ...
                            out.println("<a class=\"status1\" href=\"#\" title=\"" + paramRequest.getLocaleString("postOutLog") + "\"><strong>" + paramRequest.getLocaleString("toReview") + "</strong></a>"); //No ha sido publicado en todas las redes sociales que debiera, abrir dialogo para mostrar todos los PostOutNtes del PostOut
                        }else {
                            out.println("<a class=\"status6\" title=\""+paramRequest.getLocaleString("publish")+"\" href=\"#\"/><strong>" + paramRequest.getLocaleString("publish") + "</strong></a>");                           
                        }
                ...
                out.println("<a class=\"status3\" href=\"#\" title=\"" + paramRequest.getLocaleString("postOutLog") + "\"><strong>" + paramRequest.getLocaleString("published") + "</strong></a>");
            }
            out.println("</td>");
    ...
    private void doGenerateReport(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) {
        ...
        //SocialTopic socialTopic = (SocialTopic) SemanticObject.getSemanticObject(id).getGenericInstance();
        SocialNetwork socialNetwork = (SocialNetwork) SemanticObject.createSemanticObject(id).getGenericInstance();
        WebSite webSite = WebSite.ClassMgr.getWebSite(socialNetwork.getSemanticObject().getModel().getName());

        //HashMap hmapResult = filtros(swbSocialUser, webSite, searchWord, request, socialNetwork, page);

        //long nRec = ((Long) hmapResult.get("countResult")).longValue();
        //Iterator<PostOut> setso = ((Iterator) hmapResult.get("itResult"));
        Iterator<PostOut> setso = socialNetwork.listSocialNetworkPostOutInvs();
        ...
        try {
            ...
            createExcel(setso, paramRequest, classifyBySentiment, response, socialNetwork);
    ...
    private void createExcel(Iterator<PostOut> setso, SWBParamRequest paramRequest, String classifyBySentiment, HttpServletResponse response, SocialNetwork socialNetwork) {
        try {
            ...
            createHead(wb, row, 2, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Tema");
            createHead(wb, row, 3, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Redes");
            createHead(wb, row, 4, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Origen");
            createHead(wb, row, 5, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Creación");
            createHead(wb, row, 6, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Última actualización");
            if (classifyBySentiment != null && classifyBySentiment.equalsIgnoreCase("true")) {
                createHead(wb, row, 7, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Sentimiento");
                createHead(wb, row, 8, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Intensidad");
                createHead(wb, row, 9, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Estatus");
            } else {

                createHead(wb, row, 7, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Estatus");
            }
            ...
            while (setso != null && setso.hasNext()) {
                ...
                createCell(cellStyle, wb, troww, 2, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, postIn.getSocialTopic().getDisplayTitle(paramRequest.getUser().getLanguage()));
                ...
                createCell(cellStyle, wb, troww, 3, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, nets);

                if (postIn.getPostInSource() != null) {
                    createCell(cellStyle, wb, troww, 4, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "Origen(Image)");

                } else {
                    createCell(cellStyle, wb, troww, 4, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, "----");
                }


                createCell(cellStyle, wb, troww, 5, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, SWBUtils.TEXT.getTimeAgo(postIn.getCreated(), lang));

                createCell(cellStyle, wb, troww, 6, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER, SWBUtils.TEXT.getTimeAgo(postIn.getUpdated(), lang));
                ...
                if (classifyBySentiment != null && classifyBySentiment.equalsIgnoreCase("true")) {
                    ...
                    if (!postIn.isPublished()) {
                        ...
                        if (!isInFlow && postOutwithPostOutNets && !someOneIsNotPublished) //Se supone que por lo menos, hay publicado un PostOutNet del Post                         
                        {
                            createCell(cellStyle, wb, troww, 9, CellStyle.ALIGN_CENTER,
     ...
     */
    private HashMap filtros(String swbSocialUser, WebSite wsite, String searchWord, HttpServletRequest request, SocialNetwork socialNetwork, int nPage) {
        ...
        if (swbSocialUser != null) {
            ...
            socialTopicPostOut=Integer.parseInt(getAllPostOutbyAdminUser_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), true, socialNetwork, user));
            sQuery=getAllPostOutbyAdminUser_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork, user);
                ...
                if (nPage != 0) 
                {
                    if(request.getParameter("orderBy")!=null && (request.getParameter("orderBy").equals("viewFCPost") || request.getParameter("orderBy").equals("viewACPost")))
                    {
                        ...
                        {   //Si se selecciono que mostrara solo los que estan con FastCalendars
                            socialTopicPostOut=Integer.parseInt(getAllPostOutFC_Query(0, 0, true, socialNetwork));
                            sQuery=getAllPostOutFC_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork); 
                            aListFilter=SWBSocial.executeQueryArrayPostOuts(sQuery, wsite);
                        ...
                        {   //Si se selecciono que mostrara solo los que estan con Advance Calendars
                            socialTopicPostOut=Integer.parseInt(getAllPostOutAC_Query(0, 0, true, socialNetwork));
                            sQuery=getAllPostOutAC_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork); 
                            aListFilter=SWBSocial.executeQueryArrayPostOuts(sQuery, wsite);
                        }
                    }else if (searchWord != null && searchWord.trim().length()>0) {
                        socialTopicPostOut=Integer.parseInt(getPostOutTopicbyWord_Query(0, 0, true, socialNetwork, searchWord.trim()));
                        sQuery=getPostOutTopicbyWord_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork, searchWord.trim()); 
                    ...
                    {
                        socialTopicPostOut=Integer.parseInt(getAllPostOutSocialTopic_Query(socialNetwork));
                        if(request.getParameter("orderBy").equals("PostTypeUp"))    //Tipo de Mensaje Up
                        {
                            sQuery=getPostOutType_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        }else if(request.getParameter("orderBy").equals("PostTypeDown"))    //Tipo de Mensaje Down
                        {
                            sQuery=getPostOutType_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                            ...
                            sQuery=getPostOutSource_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                            ...
                            sQuery=getPostOutSource_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("cretedUp")) {
                            sQuery=getPostOutCreated_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("cretedDown")) {
                            sQuery=getPostOutCreated_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("sentimentUp")) {
                            sQuery=getPostOutSentimentalType_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("sentimentDown")) {
                            sQuery=getPostOutSentimentalType_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("intensityUp")) {
                            sQuery=getPostOutIntensityType_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("intensityDown")) {
                            sQuery=getPostOutIntensityType_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                            ...
                            sQuery=getPostOutUpdated_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                            ...
                            sQuery=getPostOutUpdated_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("userUp")) {
                            sQuery=getPostOutUserName_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("userDown")) {
                            sQuery=getPostOutUserName_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("statusUp")) {
                            sQuery=getPostOutStatus_Query(null, Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } else if (request.getParameter("orderBy").equals("statusDown")) {
                            sQuery=getPostOutStatus_Query("down", Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork);
                        } 
                        ...
                        if(sQuery!=null)
                        {
                           aListFilter=SWBSocial.executeQueryArrayPostOuts(sQuery, wsite);
                            ...
                            System.out.println("//No seleccionaron ningún ordenamiento!!!");
                            /*       
                            ...
                            //socialNetwork.listSocialNetworkPostOutInvs();
                            socialTopicPostOut=Integer.parseInt(getAllPostOutSocialTopic_Query(0, 0, true, socialNetwork));
                            sQuery=getAllPostOutSocialTopic_Query(Integer.valueOf((nPage * RECPERPAGE) - RECPERPAGE).longValue(), Integer.valueOf((RECPERPAGE)).longValue(), false, socialNetwork); 
                            System.out.println("THA QUERY:" + sQuery);
                            aListFilter=SWBSocial.executeQueryArrayPostOuts(sQuery, wsite);
                        }
                        ...
                        {
                            sQuery=getAllPostOutSocialTopic_Query(0L, socialTopicPostOut, false, socialNetwork); 
                            aListFilter=SWBSocial.executeQueryArrayPostOuts(sQuery, wsite);
                        }
                    }
     ...
     */
    private String getAllPostOutSocialTopic_Query(SocialNetwork socialNetwork)
    {
        String query=
           ...
           query+=
           "where {\n" +
           "  ?postUri social:socialNetwork <"+ socialNetwork.getURI()+">. \n" + 
    ...
    private String getAllPostOutSocialTopic_Query(long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutTopicbyWord_Query(long offset, long limit, boolean isCount, SocialNetwork socialNetwork, String word)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
           ...
           query+=
           "where {\n" +
           "  ?postUri social:socialTopic <"+ socialNetwork.getURI()+">. \n" + 
     ...
     */
    private String getAllPostOutbyAdminUser_Query(long offset, long limit, boolean isCount, SocialNetwork socialNetwork, User user)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutType_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutSource_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutCreated_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutUpdated_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutSentimentalType_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutIntensityType_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
    ...
    private String getPostOutUserName_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
   ...
   private String getPostOutStatus_Query(String orderType, long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
   ...
   private String getAllPostOutFC_Query(long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#>" +
   ...
   private String getAllPostOutAC_Query(long offset, long limit, boolean isCount, SocialNetwork socialNetwork)
    {
        String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#> \n" +
   ...
   private String getSumTotPostOutComments(SocialNetwork socialNetwork)
    {
        /*String query=
           "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
           "PREFIX social: <http://www.semanticwebbuilder.org/swb4/social#> \n";
           ...
           long size = SWBUtils.Collections.sizeOf(socialNetwork.listSocialNetworkPostOutInvs());           
    ...
    */
        ...
        for (int i = 0; i < postOutSocialNet.size(); i++) {
            ...
            if (postOutNet.getSocialNetwork() instanceof Twitter) {
                ...
                if (postOutNet.getStatus() == 1) {
                    ...
                    try {
                        fbComments = getRequest(params, Facebook.FACEBOOKGRAPH + postOutNet.getPo_socialNetMsgID() + "/comments",
                                Facebook.USER_AGENT);
                     //System.out.println("all the comments:" + fbComments);
                    } catch (Exception e) {
                        SocialSentPostBySocialNet.log.error("Error getting user information" , e);
                    }
                ...
                if (postOutNet.getStatus() == 1) {
                    YoutubeVideoInfo videoJsonInfo = ((Youtube) postOutNet.getSocialNetwork()).getVideoFullInfoById(postOutNet.getPo_socialNetMsgID());
                    doPrintVideo(request, response, paramRequest, out, postOutUri, 
                            socialUserExtAttr, videoJsonInfo, 
    ...
    public static void doPrintPost(Writer writer, JSONObject postsData, HttpServletRequest request,
        ...
        try {
            ...
            SocialSentPostBySocialNet.formatter.setTimeZone(TimeZone.getTimeZone("GMT-6"));
            String postType = "";
                ...
                if (!postsData.isNull("status_type")) {
                    if (postsData.getString("status_type").equals("wall_post")) {
                        ...
                        if (postsData.has("to")) {
                            ...
                            story = " to " +  "<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") +
            ...
            if (postsData.has("picture") || isAppCreated) {
               ...
               if (isAppCreated) {
                ...
                if (postType.equals("video") || postType.equals("swf")) {
                    writer.write("      <span id=\"vid" + tabSuffix + facebook.getId() + postsData.getString("id") +
                    ...
                    writer.write("      <a href=\"#\" onclick=\"hideDialog(); showDialog('" +
                            ...
                            "\" style=\"position: relative;\" onerror=\"this.src ='" + picture.replace("_n.", "_s.") +
                            "'\" onload=\"imageLoad(" + "this, 'vid" +
                    ...
                    if (postType.equals("link")) {//If the post is a link -> it has link and name
                        ...
                        writer.write("      <a href=\"#\" onclick=\"hideDialog(); showDialog('" +
                                ...
                                "'); return false;\"><img src=\"" + picture +
                                "\" style=\"position: relative;\" onerror=\"this.src ='" + picture.replace("_n.", "_s.") +
                                "'\" onload=\"imageLoad(" + "this, 'img" + tabSuffix + facebook.getId() + postsData.getString("id") +
                                "');\"/></a>");
                        writer.write("      </span>");
                    }
                }
...
//            }
            ...
            if (commentsData.length() >= 10) {
                ...
                SWBResourceURL commentsURL = paramRequest.getRenderUrl().setMode(Mode_AllComments).
            ...
            for (int i = 0; i < commentsData.length(); i++) {
                ...
                writer.write("  <a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") +
                        ...
                        renderURL.setMode("fullProfile").
                                setParameter("type", "noType").
                                setParameter("id", userId) + "','" + username +
                        "'); return false;\"><img src=\"http://graph.facebook.com/" + userId +
...
//                }
            ...
            if (actions != null && actions.length() > 0) {//Available actions for the post
                for (int i = 0; i < actions.length(); i++) {
                    if ((actions.getJSONObject(i).getString("name").equals("Comment") || postType.equals("photo") ||
                            postType.equals("video")) && socialUserExtAttr.isUserCanRespondMsg()) {//I can comment
                        writer.write("   <span class=\"inline\" id=\"" +
                                facebook.getId() + postsData.getString("id") + FacebookWall.REPLY + tabSuffix +
                                "\" dojoType=\"dojox.layout.ContentPane\">");
                        writer.write(" <a class=\"clasifica\" href=\"\" onclick=\"hideDialog(); showDialog('" +
                                renderURL.setMode(Mode_ReplyPost).
                                        setParameter("postID", postsData.getString("id")) +
                                "','Reply to " + postsData.getJSONObject("from").getString("name") +
                                "');return false;\"><span>Reply</span></a>  ");
                        writer.write("   </span>");
                    } else if (actions.getJSONObject(i).getString("name").equals("Like") ||
                            postType.equals("photo") || postType.equals("video")) {//I can like
                ...
                if ((postType.equals("photo") || postType.equals("video")) &&
                        socialUserExtAttr.isUserCanRespondMsg()) {//I can comment
                    writer.write("   <span class=\"inline\" id=\"" +
                            facebook.getId() + postsData.getString("id") + FacebookWall.REPLY + tabSuffix +
                            "\" dojoType=\"dojox.layout.ContentPane\">");
                        writer.write(" <a class=\"clasifica\" href=\"\" onclick=\"hideDialog(); showDialog('" +
            ...
            SocialSentPostBySocialNet.log.error("Error printing post:" , e);
            e.printStackTrace();
        }
    }
...
//    }
    ...
    public void doPrintVideo(HttpServletRequest request, HttpServletResponse response, 
        ...
        SocialUserExtAttributes socialUserExtAttr,YoutubeVideoInfo video, Youtube youtube) 
...
//            }
            //Obtengo informacion de video
            String videoId = video.getFirtsVideo().getId();
            YoutubeVideoElementSnippet videoSnippet = video.getFirtsVideo().getSnippet();
            YoutubeVideoElementStatistics videoStatistics = video.getFirtsVideo().getStatistics();
            YoutubeVideoElementStatus videoStatus = video.getFirtsVideo().getStatus();
            String channelId = videoSnippet.getChannelId();
            String videoTitle = videoSnippet.getTitle();
            String videoDescription = videoSnippet.getDescription();
            String videoThumbnail = videoSnippet.getThumbnail("default").getUrl() ;
            
            //Obtengo informacion de canal relacionado al video
            YoutubeChannelInfo channelInfo = youtube.getChannelFullInfoById(channelId);
            YoutubeChannelElementSnippet channelSnippet = channelInfo.getChannel().getSnippet();
            String channelTitle = channelSnippet.getTitle();
            String channelThumbnail = channelSnippet.getThumbnail("default") ;
            
            //Obtengo informacion de los comentarios relacionados al video
            YoutubeCommentThreadsInfo commentThreadsInfo = youtube.getCommentThreadsFullInfoByVideoId(videoId);
            List<YoutubeCommentThreadsElement> listCommentThreads = commentThreadsInfo.getListCommentThreads();
...
//            }
            ...
            out.write("<a href=\"#\"><img src=\"" + channelThumbnail + "\" width=\"50\" height=\"50\"/></a>");
            //out.write("<a href=\"#\"><img src=\"" + channelThumbnail.getJSONObject("entry").getJSONObject("media$thumbnail").getString("url") + "\" width=\"50\" height=\"50\"/></a>");
            out.write("<p>"  + channelTitle + "</p>");
...
//            }
            ...
            out.write("      <a href=\"#\" onclick=\"hideDialog(); showDialog('"+ paramRequest.getRenderUrl().setMode("displayVideo").setParameter("videoUrl", URLEncoder.encode("http://www.youtube.com/v/" + videoId, "UTF-8")) +
                    "','" + videoTitle + "'); return false;\"><img src=\"" + videoThumbnail + "\" style=\"position: relative;\" onerror=\"this.src ='" + videoThumbnail + "'\" onload=\"imageLoad(" + "this, 'img" + semanticYoutube.getId() + videoId + "');\"/></a>");
            out.write("      </span>");
            out.write(" </span>");
...
//                    videoDescription = video.getJSONObject("media$group").getJSONObject("media$description").getString("$t");
//                }
//            }

            out.write("<p class =\"imgdesc\">");
            out.write(videoDescription.isEmpty() ?  "&nbsp;" : videoDescription.replace("\n", "</br>"));
            out.write("</p>");
            ...
            if(videoStatus!= null ){
                if(videoStatus.getPrivacyStatus().equals(YoutubeVideoElementStatus.PRIVACY_STATUS_PRIVATE)){
                    isPrivate = true;
                }
            }
...
//                }
//            }                    
            if(listCommentThreads != null && listCommentThreads.size() > 0){
                out.write("<ul id=\"" + semanticYoutube.getId() + videoId + "/allComments\">");
                int totalComments = 0;
                totalComments = listCommentThreads.size();
...
//                }
                ...
                //Revisar funcionalidad por que obtener mas comentarios cambia
                ...
                ///////////////////////////////////////////////////////////////
                Iterator<YoutubeCommentThreadsElement> commentThreadsIterator = listCommentThreads.iterator();
                while(commentThreadsIterator.hasNext()){
                    //Datos de un camentario
                    YoutubeCommentThreadsElement commentThreadsElement = commentThreadsIterator.next();
                    YoutubeCommentThreadsElementSnippet commentThreadsSnippet = commentThreadsElement.getSnippet();
                    YoutubeCommentsElement topComment = commentThreadsSnippet.getTopComment();
                    String comentarioId = topComment.getId();
                    YoutubeCommentsElementSnippet commentsSnippet = topComment.getSnippet();
                    
                    //for(int c = 0; c < listCommentThreads.size(); c++){                            
                    totalComments++;
...
//                    }
                    ...
                    out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", commentsSnippet.getauthorChannelId()) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\"><img src=\"" +commentsSnippet.getauthorImageUrl() + "\" width=\"50\" height=\"50\"/></a>");

                    out.write("<p>");
                    //out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", comment.getJSONArray("author").getJSONObject(0).getJSONObject("yt$userId").getString("$t")) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\">" + comment.getJSONArray("author").getJSONObject(0).getJSONObject("name").getString("$t") + "</a>:");
                    out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", commentsSnippet.getauthorChannelId()) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\">" + commentsSnippet.getAuthorName() + "</a>:");
                    
                    //out.write(       comment.getJSONObject("content").getString("$t").replace("\n", "</br>"));
                    out.write(commentsSnippet.getText().replace("\n", "</br>"));
                    ...
                    Date date = formatter.parse(commentsSnippet.getpublishedDate());
                    ...
                    out.write(" <a href=\"\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("commentComment").setParameter("suri", objUri).setParameter("videoId", videoId).setParameter("commentId", comentarioId) + "','Comment to " + commentsSnippet.getText().replace("\n", "</br>") + "');return false;\">Comment</a>");
        ...
        //Comments
            ...
            Date date = formatter.parse(videoSnippet.getPublishedDate());

            out.write("<em>" + YoutubeWall.humanFriendlyDate(date, paramRequest) + "</em>");


//            if(video.get.isNull("yt$statistics")){
//                out.write(paramRequest.getLocaleString("views") + ":" + video.getJSONObject("yt$statistics").getInt("viewCount") + " ");
//            }
            if(videoStatistics != null){
                out.write(paramRequest.getLocaleString("views") + ":" + videoStatistics.getViewCount() + " ");
            }
...
//                }
            ...
            // Obtengo comentarios
            List<YoutubeCommentThreadsElement> listCommentThreads = youtube.getCommentThreadsFullInfoByVideoId(videoId).getListCommentThreads();
            int totalComments = listCommentThreads.size();
...
//                    }
                    ...
                    if(listCommentThreads != null && listCommentThreads.size()> 0){//Only print <li></li> because the HTML will be returned inside <ul></ul
                        int commentCounter = 0;
                        Iterator<YoutubeCommentThreadsElement> commentIterator = listCommentThreads.iterator();
                        while(commentIterator.hasNext()){
                            //for(int c = 0; c < arrayComments.length(); c++){
                            YoutubeCommentThreadsElement commentThreads = commentIterator.next();
                            YoutubeCommentsElement topComment = commentThreads.getSnippet().getTopComment();
                            YoutubeCommentsElementSnippet commentSnippet = topComment.getSnippet();
                            commentCounter++;
//                            JSONObject comment = arrayComments.getJSONObject(c);
//                            JSONObject usrCommentProfile = null;
//                            if(!comment.isNull("author")){
...
//                            }
                            out.write("<li>");
                            //out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", comment.getJSONArray("author").getJSONObject(0).getJSONObject("yt$userId").getString("$t")) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\"><img src=\"" + usrCommentProfile.getJSONObject("entry").getJSONObject("media$thumbnail").getString("url") + "\" width=\"50\" height=\"50\"/></a>");
                            out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", commentSnippet.getChannelId()) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\"><img src=\"" + commentSnippet.getauthorImageUrl() + "\" width=\"50\" height=\"50\"/></a>");

                            out.write("<p>");
                            //out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", comment.getJSONArray("author").getJSONObject(0).getJSONObject("yt$userId").getString("$t")) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\">" + comment.getJSONArray("author").getJSONObject(0).getJSONObject("name").getString("$t") + "</a>:");
                            out.write("<a href=\"#\" title=\"" + paramRequest.getLocaleString("viewProfile") + "\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("showUserProfile").setParameter("id", commentSnippet.getChannelId()) + "','" + paramRequest.getLocaleString("viewProfile") + "'); return false;\">" + commentSnippet.getAuthorName() + "</a>:");
                            
                            //out.write(       comment.getJSONObject("content").getString("$t").replace("\n", "</br>"));
                            out.write(commentSnippet.getText().replace("\n", "</br>"));
                            ...
                            out.write("<span>");
                            //Date date = formatter.parse(comment.getJSONObject("published").getString("$t"));
                            Date date = formatter.parse( commentSnippet.getpublishedDate() );
                           ...
                           // out.write(" <a href=\"\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("commentComment").setParameter("suri", suri).setParameter("videoId",videoId).setParameter("commentId", comentarioId.substring(comentarioId.indexOf("comment") + 8)) + "','Comment to " + comment.getJSONObject("content").getString("$t").replace("\n", "</br>") + "');return false;\">Comment</a>");
                            out.write(" <a href=\"\" onclick=\"hideDialog(); showDialog('" + paramRequest.getRenderUrl().setMode("commentComment").setParameter("suri", suri).setParameter("videoId",videoId).setParameter("commentId", comentarioId.substring(comentarioId.indexOf("comment") + 8)) + "','Comment to " + commentSnippet.getText().replace("\n", "</br>") + "');return false;\">Comment</a>");
                ...
                SocialSentPostBySocialNet.log.error("Problem getting more comments", e);
            }
        }
    }
...
//                JSONObject jsonVideo = new JSONObject(ytVideo);
                YoutubeVideoInfo videoFullInfoById = ((Youtube)socialNetwork).getVideoFullInfoById(videoId);
                YoutubeVideoElementSnippet videoSnippet = videoFullInfoById.getFirtsVideo().getSnippet();
                YoutubeChannelInfo channelFullInfoById = ((Youtube)socialNetwork).getChannelFullInfoById(videoSnippet.getChannelId());
                YoutubeChannelElementSnippet channelSnippet = channelFullInfoById.getChannel().getSnippet();
                
                String title = videoSnippet.getTitle();
                String description = videoSnippet.getDescription();
                String creatorName = channelSnippet.getTitle();
                String creatorId =  channelFullInfoById.getChannel().getId();
    ...
    public void doShowFullProfile(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (profileType.equals("noType")) {
            try {
                ...
                SocialSentPostBySocialNet.log.error("Error getting profile information" + jsone);
                return;
            }
        }
