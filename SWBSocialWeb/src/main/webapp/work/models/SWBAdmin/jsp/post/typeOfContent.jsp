...
%>
                ...
                <div class="campo"><%=postOut == null ? postOutNull : postOutNNull%><br> <%if (postOut == null) {%><input  id="<%=id%>_Text" name="<%=id%>_Text" type="text" size="4" class="nobord" readonly ><label class="labelInfo"><img class="swbIconTwitter" src="/swbadmin/css/images/trans.png"/> 140  <img class="swbIconFacebook" src="/swbadmin/css/images/trans.png"/> 2000  <img class="swbIconTumblr" src="/swbadmin/css/images/trans.png"/> 5000  </label><%} else {%><input id="<%=id%>_Text" name="<%=id%>_Text" type="text" size="4" class="nobord" readonly  value="<%=value.length()%>"><label class="labelInfo"><img class="swbIconTwitter" src="/swbadmin/css/images/trans.png"/> 140  <img class="swbIconFacebook" src="/swbadmin/css/images/trans.png"/> 2000  <img class="swbIconTumblr" src="/swbadmin/css/images/trans.png"/> 5000  </label><%}%></div>
                </p>
                <!--Documents-->
                 <p>
                    <%
                ...
                <div id="<%=objUri%><%=sourceCall%>pub-calendar" dojoType="dijit.TitlePane" title="Calendarizar" open="false" duration="150" minSize_="20" splitter_="true" region="bottom">
                    <div class="calendar-fast">
                        ...
                        %>
                        <input type="hidden" id="<%=objUri%><%=sourceCall%>_today_hidden" name="today_hidden" value="<%=year+"-" +month+"-"+day%>"/>
                        <div>Día:<input type="text" name="postOut_inidate" id="<%=objUri%><%=sourceCall%>_postOut_inidate" dojoType="dijit.form.DateTextBox"  size="11" style="width:110px;" hasDownArrow="true" value="<%=date%>" constraints="{min:'<%=year%>-<%=String.format("%02d", month)%>-<%=String.format("%02d", day)%>'}" onchange="removeMin(this, '<%=objUri%><%=sourceCall%>_postOut_starthour', document.getElementById('<%=objUri%><%=sourceCall%>_today_hidden').value, '<%=hour%>', '<%=minute%>');"/>
                            Hora:<input dojoType="dijit.form.TimeTextBox" name="postOut_starthour" id="<%=objUri%><%=sourceCall%>_postOut_starthour"  value="<%=(starthour != null && starthour.trim().length() > 0 ? "T" + starthour + ":00" : "")%>" constraints={formatLength:'short',selector:'timeOnly',timePattern:'HH:mm',min:'T<%=String.format("%02d", hour)%>:<%=String.format("%02d", minute)%>:00'} disabled="true" />
                        </div>
                    </div>
            ...
            <form dojoType="dijit.form.Form" id="<%=objUri%><%=sourceCall%>frmUploadPhoto" action="<%=urlAction.setAction("uploadPhoto")%>" method="post" onsubmit="submitForm('<%=objUri%><%=sourceCall%>frmUploadPhoto');return false;">
                ...
                <div class="pub-redes">
                   ...
                   <!-- <div class="campo"><%=postOut == null ? photoMgr.renderElement(request, Photo.social_msg_Text, photoMgr.MODE_CREATE) : photoMgr.renderElement(request, Photo.social_msg_Text, photoMgr.MODE_EDIT)%></div>-->
                    <div class="campo"><%=postOut == null ? create : edit%> <%if (postOut == null) {%><input  id="<%=id%>_Text" name="<%=id%>_Text" type="text" size="4"class="nobord" readonly > <label class="labelInfo"><img class="swbIconTwitter" src="/swbadmin/css/images/trans.png"/> 140  <img class="swbIconFacebook" src="/swbadmin/css/images/trans.png"/> 2000  <img class="swbIconTumblr" src="/swbadmin/css/images/trans.png"/> 5000  </label><%} else {%><br><input id="<%=id%>_Text" name="<%=id%>_Text" type="text" size="4" class="nobord" readonly value="<%=value.length()%>"><label class="labelInfo"><img class="swbIconTwitter" src="/swbadmin/css/images/trans.png"/> 140  <img class="swbIconFacebook" src="/swbadmin/css/images/trans.png"/> 2000  <img class="swbIconTumblr" src="/swbadmin/css/images/trans.png"/> 5000 </label><%}%></div>
                    </p>
                    <p>
                        <%
                            if (postOut != null) {
        ...
        <div class="swbform">
              <form dojoType="dijit.form.Form" id="<%=objUri%><%=sourceCall%>frmUploadVideo" action="<%=urlAction.setAction("uploadVideo")%>" method="post" onsubmit="submitForm('<%=objUri%><%=sourceCall%>frmUploadVideo');return false;">
                ...
                <div class="pub-redes">
                    ...
                    <div class="campo"><%=create%><br><input id="<%=id%>_Text" name="<%=id%>_Text" type="text" size="4" class="nobord" readonly value="<%=value.length()%>"><label class="labelInfo"><img class="swbIconTwitter" src="/swbadmin/css/images/trans.png"/> 140  <img class="swbIconFacebook" src="/swbadmin/css/images/trans.png"/> 2000  <img class="swbIconTumblr" src="/swbadmin/css/images/trans.png"/> 5000 <img class="swbIconYouTube" src="/swbadmin/css/images/trans.png"/> 5000  </label></div>
                    </p>
                    <p>
