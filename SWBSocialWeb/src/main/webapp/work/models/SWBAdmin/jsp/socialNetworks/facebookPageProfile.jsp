...
<%!
    public static String getFullPageProfileFromId(String id, Facebook facebook){
        ...
        //fan_count eliminado de la lista de datos esperados v2.3
        params1.put("fields", "about,company_overview,description,release_date,general_info");
...
<%
    ...
    if (pageResp != null) {
        ...
        JSONObject information = pageResp;
...
%>
    <fieldset>
        ...
        <div align="left">
             ...
             <a href="http://www.facebook.com/profile.php?id=<%=locationId%>" title="<%=paramRequest.getLocaleString("viewLocFB")%>"  target="_blank"><%=locationName%></a>
        </div>
<%
            if (!locationCoordinates.isEmpty()) {%>
        <div align="left">
            <%=paramRequest.getLocaleString("viewOnGoogle")%>:
            <a href="https://maps.google.com/maps?q=<%=locationCoordinates%>" title="<%=paramRequest.getLocaleString("viewLocGoogle")%>"  target="_blank">(<%=locationCoordinates%>)</a>
        </div>
<%          }%>
    </fieldset>
<%
