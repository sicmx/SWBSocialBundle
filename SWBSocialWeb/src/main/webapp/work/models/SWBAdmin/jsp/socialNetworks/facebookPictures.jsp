...
//                "( SELECT filter_key FROM stream_filter WHERE uid = me() AND name = 'Photos') ORDER BY created_time DESC LIMIT 50\", \"usernames\": \"SELECT uid, name FROM user WHERE uid IN (SELECT actor_id FROM #pictures)\", \"pages\":\"SELECT page_id, name FROM page WHERE page_id IN (SELECT actor_id FROM #pictures)\"}");
        params.put("fields", "id,from,created_time,likes.sumary(true),images,name,name_tags,message,type,comments.limit(5).summary(true)");
        params.put("limit", "" + picturesExpected);
        //El borde photos, obtiene informacion de las fotos en las que el usuario autenticado ha sido etiquetado
        String fbResponse = getRequest(params, Facebook.FACEBOOKGRAPH + "me/photos",
                        Facebook.USER_AGENT);
        
        //Gets the newest post and saves the ID of the last one
        String createdTime = picture(fbResponse, out, true, request, paramRequest, model);
        SWBResourceURL renderURL = paramRequest.getRenderUrl().
                setParameter("suri", objUri).setParameter("currentTab", PICTURES_TAB);
    /*
...
%>
    <div id="<%=objUri%>getMorePictures" dojoType="dijit.layout.ContentPane">
        <div align="center" style="margin-bottom: 10px;">
            <label id="<%=objUri%>morePicturesLabel">
                <a href="#" onclick="appendHtmlAt('<%=renderURL.setMode("getMorePictures").setParameter("createdTime", createdTime).setParameter("currentTab", PICTURES_TAB)%>','<%=objUri%>getMorePictures', 'bottom');try{this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode);}catch(noe){}; return false;">
                    <%=paramRequest.getLocaleString("getMoreImages")%>
                </a>
            </label>
        </div>
    </div>
