...
<%
    ...
    String activitiesPath = paramRequest.getRenderUrl().setMode("getActivities").
    ...
    String peoplePath = paramRequest.getRenderUrl().setMode("getPeople").
                          setCallMethod(SWBParameters.Call_DIRECT).
                          setParameter("suri", URLEncoder.encode(request.getParameter("suri"))).
                          setParameter("contentTabId", HOME_TAB).toString();
%>
    ...
    <!--TODO:Modificar este codigo para recarga de clases, posible cambio por onLoad -->
        ...
        <!-- usar un modo del recurso para llenar cada tab width: 650px !important; width: 400px !important; -->
        <div class="pub-redes" style="width:650px !important; height:500px;" id="<%=objUri + HOME_TAB%>" dojoType="dijit.layout.ContentPane" title="<%=paramRequest.getLocaleString("myNovelties")%>" refreshOnShow="false" href="<%=activitiesPath%>" _loadingMessage="<%=loading%>" style="overflow:auto;" style_="border:0px; width:100%; height:100%" onLoad_="onLoadTab(this);">
        </div>
        <div class="pub-redes" style="height:500px;" id="<%=objUri + FOLLOWERS_TAB %>" dojoType="dijit.layout.ContentPane" title="<%=paramRequest.getLocaleString("myFollowers")%>" refreshOnShow="false" href="<%=peoplePath%>" _loadingMessage="<%=loading%>" style="overflow:auto;" style_="border:0px; width:100%; height:100%" onLoad_="onLoadTab(this);">
        </div>
